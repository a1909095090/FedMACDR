...
from FedMaclient import *
from Manifold import *
from Fedl import *
from Maserver import *
def arg_parse():
    ...
    parser.add_argument("--malr", type=float, default=0.1,
...
def main():
    ...
    clients = [MaClient(Manifold, c_id, args,
                          train_datasets[c_id], valid_datasets[c_id],
                          test_datasets[c_id],all_adj=all_adjs[c_id])
                   for c_id in range(n_clients)]
    # Initialize the aggretation weight
    init_clients_weight(clients)
    ...
    #     server = Server(args)
    # server= Server(args)
    server = MaServer(args, True)
    run_fedl(clients, server, args)

# class InitsigleDoaminMa:
#     def __init__(self,ckpt_filename,domain):
#
...
#         # 加载数据集 里边有所有域的数据集 每个域的的数据集包括 user_ids, items, user_items, num_users, num_items
#         train_datasets, valid_datasets, test_datasets = load_ratings_dataset(args)
#         self.uG = UserGraph(args, domain=domain, file_name=domain + '_full_g1.npy')
...
#         self.uemb=self.model.user_item_emb
#
#
#         self.full_g= self.uG.get_users_adjacency_matrix()
#         # np.save('Food_full_g1.npy', Food_full_g)
#         self.D=self.uG.get_D()
#         self.L=self.uG.get_Laplacian()
#
#         self.Mamodel=Manifold(self.uG,args)
#
...
#
#         self.opt.zero_grad()
#         loss,decoder_l,encoder_l= self.Mamodel.loss()
#         loss.backward()
#
#         print(self.Mamodel.encoder.fc1.weight)
#         print(decoder_l)
#         # if epoch % 100 ==0:
#         #     print("epoch :",epoch)
...
#         """
#         users=np.arange(self.uG.num_users)
...
#
#
#         _, v = self.model(users, all_items)
#         u=self.Mamodel.get_target_domain_ue(users)
#
#
#
...
#             # then `...[0].item()` indicates the ranking of the positive sample.
...
#                                   "_" + "1732693711",
#                                  )
#     # Food_Model=InitsigleDoaminMa(ckpt_filename+"/client0.pt",domain="Food")
#     # Kitchen_Model=InitsigleDoaminMa(ckpt_filename+"/client1.pt",domain="kitchen")
#     domains=["Food","Kitchen","Clothing","Beauty"]
#     clients = [InitsigleDoaminMa((ckpt_filename+f"/client{c_id}.pt"), domain)
#                for c_id ,domain in enumerate(domains)]
#     print(domains[0].model)
    # clients = [Client(ModelTrainer, c_id, args,
    #                   train_datasets[c_id], valid_datasets[c_id],
    #                   test_datasets[c_id],
    #                   all_adj=all_adjs[c_id]) for c_id in range(n_clients)]
    # opt = optim.Adam(clients[1].Mamodel.parameters(), lr=0.1, weight_decay=0.001)
    # for epoch in range(1000):
    #     opt.zero_grad()
    #     loss,decoder_l,_= clients[1].Mamodel.loss()
    #     loss.backward()
    #     print("epoch :", epoch)
    #     print(clients[1].Mamodel.encoder.fc1.weight)
    #     print(decoder_l)
    #
    #     print("-"*40)
    #     opt.step()
...
if __name__ == "__main__":
    ...
    # # 设置日志文件
    # init_logger(args)
    # # ckpt_filename = os.path.join("./checkpoint/domain_", "FKCB", "LocalGNN_", "1732693711", "client0
    #
    # # pre_trainer(args) #预训练单域。
    # # trainFedMa()
    # ckpt_filename = os.path.join("./checkpoint/domain_", "FKCB", "LocalGNN_", "1732693711", "client0.pt")
    # # dir = os.path.join("checkpoint", "domain_FKCB", "LocalGNN" + "_" + "1732693711", "Food_ug.pt")
    #
    ...
    # train_datasets, valid_datasets, test_datasets = load_ratings_dataset(args)
    # domain='food'
    # uG = UserGraph(args, domain=domain, file_name=domain + '_full_g1.npy')
    # model = GNN(uG.num_users, uG.num_items,args)
