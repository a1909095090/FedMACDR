...
class Client:
    ...
    def train_epoch(self, round, args, global_params=None):
        ...
        for epoch in range(args.local_epoch):
            ...
            for user_ids, interactions in self.train_dataloader:
                ...
                loss += batch_loss
                step += 1
            # if epoch == args.local_epoch-1 and "GNN" not in args.method:
            #
            #     self.save_disc_params()
            self.save_disc_params()
            gc.collect()
    ...
    def save_disc_params(self):
        ...
        params = self.trainer.discri.state_dict()
        try:
            torch.save(params, ckpt_filename)
            print("Modeldisc saved to {}".format(ckpt_filename))
        except IOError:
            print("[ Warning: Saving disc failed... continuing anyway. ]")